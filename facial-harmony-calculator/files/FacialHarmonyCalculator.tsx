...
type NPt = { x: number; y: number }; // normalised 0..1 point
...
/* Convert normalised points to aspect-correct pixel space for trig. */
...
export default function FacialHarmonyCalculator() {
  ...
  // current measured values, by metric id
  ...
  /* recompute photo value whenever points for the active metric change */
  useEffect(() => {
      ...
      {!imgSrc ? (
        ...
        <>
          ...
          >
            ...
            {/* connecting lines + points overlay */}
            <svg
            ...
            {/* point markers (real DOM so they stay round) */}
            {pts.map((p, i) => (
              <span
                key={i}
